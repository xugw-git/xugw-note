...
>
> 参考文章：<https://blog.csdn.net/zinchliang/article/details/120262185>

## 更新内容

增加对交替填充背景色的支持、字体颜色格式使用十六进制替代RGB
...
var setBorder = function (lucksheetfile, worksheet) {
  ...
  for (let x in borderInfoCompute) {
    ...
    let row = parseInt(x.substring(0, x.indexOf('_')))
    let column = parseInt(x.substring(x.indexOf('_') + 1))
    if (info.t != undefined) {
